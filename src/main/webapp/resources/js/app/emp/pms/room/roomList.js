			...
			$.each(dataList, function(index, room){
				...
				if(room.rmClean == '청소요청'){
					rmCleanButton = `
						<button class="btn badge badge rounded-pill badge-subtle-primary roomClean btn-sm" 
								type="button" data-value1="${room.rmNo }" data-value2="${room.rmClean}">청소완료</button>
					`
				}else{
					rmCleanButton = `
					`
				...
				trTags += `
					<tr>
						...
						<td class="rmtTypeCd align-middle ">${room.rmtTypeCd }</td>
...
$(document).ready(function () {
	...
	$(".selectOneSearchWord").click(function() {
