				...
				$.each(annualDataList, function(index, oprtnAnnual){
					...
					trTags += `
						<tr>
							...
							<td class="anlLeaUseNo">${oprtnAnnual.anlLeaUseNo}</td>
							...
							<td class="rqstSe">${oprtnAnnual.rqstSe}</td>
		...
		if(fxtrsDataList.length > 0) {
				$.each(fxtrsDataList, function(index, oprtnFxtrs){
					...
					if (oprtnFxtrs.rqstSe  == '승인'){
						...
						`;
					}else if (oprtnFxtrs.rqstSe == '구매승인'){
						statusBadge = `
